...
export class UsuarioModelo {
  static async getAll ({ orderby, pagina, registros }: { orderby?: string; pagina?: number; registros?: number }) {
    let lowerCaseOrderBy
    let paginaNumero = 1
    let registrosNumero: string = process.env.REGISTROS_MOSTRAR_MAX
    if (orderby) {
